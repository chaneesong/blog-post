  ...
  try {
    ...
    if (type === MODIFIED) {
      res = await axios.patch(
        `${process.env.LOCAL_SERVER_URL}/posts/${postData.title}`,
        postData
      );
    } else if (type === ADDED) {
      res = await axios.post(`${process.env.LOCAL_SERVER_URL}/posts`, postData);
    } else if (type === DELETED) {
      res = await axios.delete(
        `${process.env.LOCAL_SERVER_URL}/posts/${postData.id}`
      );
    }
    return res.data;
  } catch (error) {
